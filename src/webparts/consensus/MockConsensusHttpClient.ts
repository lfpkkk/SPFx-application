import { ISPRequirement, ISPRequirementList, ISPScores, ISPScoresList } from "./consensus-entities";

export default class MockConsensusHttpClient {

    private static _items: ISPRequirement[] = [
        {
    ...
    public getRequirements() : Promise<ISPRequirementList> {
        ...
        return new Promise<ISPRequirement[]>((resolve)=> {
            resolve(MockConsensusHttpClient._items);
        }) .then((data: ISPRequirement[]) => {
            let listData: ISPRequirementList = { value: data };
            return listData;
    });

}

private static _scores: ISPScores[] = [
    ...
    public getScoresForRequirementNum(reqNum, vendorId) : Promise<ISPScoresList> {

        return new Promise<ISPScores[]>((resolve)=> {
            resolve(MockConsensusHttpClient._scores);
        }) .then((data: ISPScores[]) => {
            let listData: ISPScoresList = { value: data };
            return listData; 
    });
}


public getScoresForVendorId(vendorId) : Promise<ISPScoresList> {
    return null;
}
